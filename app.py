...
def handle_command():
    try:
        ...
        # Початкова ініціалізація сесії
        if username not in active_sessions:
            profile = player_manager.load_profile(username)
            is_new = not bool(profile.password_hash)
            ...
            # Підказка користувачу в залежності від того, чи існує акаунт
            msg = (f"\n[НОВИЙ КОРИСТУВАЧ]: Користувача '{username}' не знайдено.\n"
                   f"Придумайте та введіть новий пароль для реєстрації:") if is_new else (
                   f"\n[АВТОРИЗАЦІЯ]: Знайдено профіль '{username}'.\n"
                   f"Введіть пароль доступу:")
        ...
        if state == "ASK_PASSWORD":
            ...
            if is_new:
                ...
                user_data["session"] = GameSession(profile, case_data)
                user_data["state"] = "MENU"
                
                return jsonify({
                    "result": f"\n[РЕЄСТРАЦІЯ УСПІШНА]: Профіль для '{username}' успішно створено та збережено!\nВведіть команду 'start' для перегляду головного меню справи.",
                ...
                if profile.password_hash == hash_password(cmd):
                    ...
                    if remaining <= 0:
                        ...
                        return jsonify({
                            "result": "\n[КРИТИЧНА ТРИВОГА]: СУВЕРЕННИЙ ЗАХИСТ АКТИВОВАНО! Виявлено 3 невдалі спроби несанкціонованого доступу. Термінал заблоковано!",
                            "rank": "",
                            "score": 0,
                            "name": "",
                            "locked": True
